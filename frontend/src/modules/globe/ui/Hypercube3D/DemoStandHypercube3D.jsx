...
import styles from "./DemoStandHypercube3D.module.css"
...
/**
 * Гиперкуб для `/demo/face`: компоновка под HUD-панель.
 */
export default function DemoStandHypercube3D({
	onOpenBpm,
	highlightCaseTree,
	hudPanelExpanded = false,
}) {
	const model = useHypercube3DModel({ onOpenBpm, highlightCaseTree })
	return (
		<Hypercube3DWorkspace
			styles={styles}
			model={model}
			footer={INSTRUCTIONS}
			compactHud={!hudPanelExpanded}
			demoFloatHudInfo
			rootExtraClass={!hudPanelExpanded ? styles.rootHudCompact : ""}
		/>
	)
}
