...
const CHART_YEAR_TICKS = [
	2020, 2022, 2024, 2026, 2028, 2030, 2032, 2034, 2036, 2038, 2040, 2042, 2044, 2046, 2050, 2054, 2058, 2063,
	2065,
]
...
function XAxisTickWithLine({ x, y, payload }) {
	...
	return (
		<g transform={`translate(${x},${y})`}>
			<text x={0} y={14} textAnchor="middle" fill="#a0aec0" fontSize={10}>
				{payload.value}
			</text>
		</g>
	)
	...
	return (
		<g>
			...
			{(() => {
				...
				return (
					<text x={p2065.x} y={y0 + 22} textAnchor="middle" fill="#a0aec0" fontSize={10}>
						{END_YEAR}
					</text>
				)
			})()}
			...
			{CHART_YEAR_TICKS.slice(0, -1).map((t, i) => {
				...
				return (
					<ReferenceArea
						...
						x1={t}
						x2={next}
						fill={isHist ? HISTORY_COLOR : FORECAST_COLOR}
						fillOpacity={0.25}
					/>
				)
			})}
...
function CashFlowChart({ faceSeed = 0 }) {
	...
	return (
		<div className={styles.cashflowContainer}>
			<div className={`${styles.cashflowControls} ${styles.cashflowControlsFull}`}>
				<div className={`${styles.cashflowControlGroup} ${styles.cashflowControlGroupFlex}`}>
					<label className={styles.cashflowSliderLabel}>
				...
				<div className={`${styles.cashflowControlGroup} ${styles.cashflowControlGroupFlex}`}>
					<label className={styles.cashflowSliderLabel}>Период прогноза (лет): {years}</label>
					<input
						type="range"
						min="5"
			...
			<div className={styles.cashflowCharts}>
				<div className={styles.chartContainer}>
					...
					<ResponsiveContainer width="100%" height={340}>
						<AreaChart
							...
							margin={{ top: 10, right: 32, left: 10, bottom: 28 }}
						>
							<defs>
							...
							<Area
								type="monotone"
								dataKey="cashFlowHistory"
								stroke={HISTORY_COLOR}
								fill="url(#colorCashFlowHist)"
								fillOpacity={1}
								connectNulls
								name="Cash Flow (история)"
								dot={{ fill: HISTORY_COLOR, r: 3 }}
								isAnimationActive={false}
							/>
							<Area
								type="monotone"
								...
								fill="url(#colorCashFlowFcast)"
								fillOpacity={1}
								connectNulls
								name="Cash Flow (прогноз)"
				...
				<div className={styles.chartContainer}>
					...
					<ResponsiveContainer width="100%" height={340}>
						...
						>
							...
							<Area
								type="monotone"
								dataKey="productionHistory"
								stroke={HISTORY_COLOR}
								fill="url(#colorProdHist)"
								fillOpacity={1}
								connectNulls
								name="Добыча (история)"
								dot={{ fill: HISTORY_COLOR, r: 3 }}
			...
			<div className={styles.cashflowPeriodLegend}>
				<span className={styles.cashflowLegendHistory} style={{ color: HISTORY_COLOR }}>
					● История (до {CURRENT_YEAR})
				</span>
				<span className={styles.cashflowLegendForecast} style={{ color: FORECAST_COLOR }}>
					● Прогнозный период (после {CURRENT_YEAR})
				</span>
			</div>
			<p className={styles.cashflowPrognozHint}>Красная линия — текущий срез {CURRENT_YEAR} г.</p>

			{breakEvenPoint && (
				<div className={styles.breakEvenAlert}>
					<h4>Выход за рамки профиля</h4>
					<p>
						При текущих параметрах Cash Flow упадет ниже 50% от базового значения к{" "}
						<strong>{breakEvenPoint}</strong> году.
					</p>
			...
			<div className={styles.cashflowSummary}>
				<div className={styles.summaryItem}>
					<span className={styles.summaryLabel}>Суммарный Cash Flow:</span>
					<span className={styles.summaryValue}>
						{chartData.reduce((sum, d) => sum + d.cashFlow, 0).toLocaleString("ru-RU")} млн руб
					</span>
				</div>
				<div className={styles.summaryItem}>
					<span className={styles.summaryLabel}>Средний Cash Flow:</span>
					<span className={styles.summaryValue}>
						{(
							chartData.reduce((sum, d) => sum + d.cashFlow, 0) / (chartData.length || 1)
						).toLocaleString("ru-RU")}{" "}
						млн руб/год
					</span>
				</div>
				<div className={styles.summaryItem}>
					<span className={styles.summaryLabel}>NPV (приведенная стоимость):</span>
					<span className={styles.summaryValue}>
